...
const Main = () => {
    ...
    useEffect(() => {
        ...
        freecurrencyapi.latest({
            base_currency: 'USD',
            currencies: 'TRY,USD,EUR,GBP,CHF,RUB,CZK,AUD,SGD,DKK,CAD,JPY,PLN'
        }).then(response => {
            if (response && response.data) {
                const data = response.data;
                const formattedData = Object.entries(data)
                    .map(entry => `${entry[0]}: ${entry[1].toFixed(6)}`)
                    .join(' | ');
    ...
    return (
        <>
          <div>
            <AppBar position="fixed"> {/* position="fixed" kullanarak AppBar'ı üstte sabitleyin */}
